...
from groupy.garray.p3_array import P3Array
from groupy.garray.Z2_array import Z2Array


class C3Array(GArray):
    ...
    def __init__(self, data, p='axial'):
        ...
        self._left_actions[P3Array] = self.__class__.left_action_axial_P3
        self._left_actions[C3Array] = self.__class__.left_action_axial
        super(C3Array, self).__init__(data, p)
...
def identity(shape=(), p='axial'):
    e = C3Array(np.zeros(shape + (1,), dtype=np.int), 'axial')
...
def rand(size=()):
    ...
    return C3Array(data=data, p='axial')
