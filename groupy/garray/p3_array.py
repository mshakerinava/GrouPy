...
class P3Array(GArray):
    ...
    def __init__(self, data, p='axial'):
        ...
        self._left_actions[P3Array] = self.__class__.left_action_axial
        super(P3Array, self).__init__(data, p)

    def left_action_axial(self, other):
        self_axial = self.reparameterize('axial').data
        other_axial = other.reparameterize('axial').data

        self_r = self_axial[..., 0] * 2
        self_u = self_axial[..., 1]
        self_v = self_axial[..., 2]

        other_r = other_axial[..., 0] * 2
...
def meshgrid(r=r_range(), u=u_range(), v=v_range()):
    r = P3Array(r.data[:, None, None, ...], p=r.p)
    u = P3Array(u.data[None, :, None, ...], p=u.p)
    v = P3Array(v.data[None, None, :, ...], p=v.p)
    return u * v * r

C3 = P3Array([[0, 0, 0], [1, 0, 0], [2, 0, 0]])


def randC3():
    a = np.random.randint(0, 3)
    return P3Array([a, 0, 0])
